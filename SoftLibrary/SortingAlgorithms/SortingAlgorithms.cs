    ...
    public static class SortingAlgorithms<T> where T : IComparable<T>
    {
        ...
        {            
            ...
            while (end > 0)
            {
                ...
                ShiftDown(collection, 0, end);
            }
        }

        // method puts the elements of the collection in a heap order, with biggest element at root position
        private static void Heapify(IList<T> collection)
        {
            var start = (collection.Count - 2) / 2;

            while (start >= 0)
            {
                ShiftDown(collection, start, collection.Count - 1);
                start--;
            }
        }

        public static void ShiftDown(IList<T> collection, int start, int end)
        {
            var root = start;
